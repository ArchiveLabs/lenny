...
# Rate limit window in seconds - ensures TTL compatibility across all rate limit zones
# This value should match across any external rate limiting layers (e.g., Nginx, CDN)
# Using 60 seconds (1 minute) as the standard window
...
# Note: default_limits=[] means rate limiting must be explicitly applied per endpoint
# This ensures OPDS and /items endpoints have no rate limiting as required
limiter = Limiter(
